...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "input.txt")) as gameFile:
    for gameLine in gameFile:
        ...
        playerBGameAction = gameActionFactory.createPlayerBGameAction(playerBSymbol)

        answer += playerBGameAction.pointsAwarded
        answer += GameRound(playerAGameAction, playerBGameAction).getResult().getPointsAwarded()
